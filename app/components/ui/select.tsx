...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "bg-transparent flex h-10 w-full items-center justify-between rounded-md border border-input px-3 py-2 text-sm shadow-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50",
      className,
    )}
    {...props}
  >
...
function FormSelect(props: Props) {
  ...
  const f = getInputProps({});
  return (
    <div className="space-y-1">
      <Label htmlFor={f.name}>{props.label}</Label>
      <Select onValueChange={f.onChange} defaultValue={f.defaultValue} name={f.name}>
        <SelectTrigger>
